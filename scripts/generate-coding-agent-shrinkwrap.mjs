...
const allowedInstallScriptPackages = new Map([
	...
	["protobufjs@7.5.8", "postinstall only warns about protobufjs version scheme mismatches"],
]);
...
function addInternalWorkspace(shrinkwrapPackages, addedPaths, queue, name, workspace) {
	...
	for (const dependencyName of Object.keys(packageJson.dependencies ?? {})) {
		queue.push({ name: dependencyName, from: outputPath, optional: false });
	}
	for (const dependencyName of Object.keys(packageJson.optionalDependencies ?? {})) {
		queue.push({ name: dependencyName, from: outputPath, optional: true });
	}
}

function addExternalPackage(lockPackages, shrinkwrapPackages, addedPaths, queue, name, from, optional = false) {
	let lockPath;
	try {
		lockPath = resolveExternalDependency(lockPackages, name, from);
	} catch (err) {
		if (optional) {
			// Platform-specific optional dep not installed on this OS — skip.
			return;
		}
		throw err;
	}
	...
	for (const depName of Object.keys(deps)) {
		queue.push({ name: depName, from: lockPath, optional: false });
	}
	for (const depName of Object.keys(optDeps)) {
		queue.push({ name: depName, from: lockPath, optional: true });
	}
}
...
function validateShrinkwrap(shrinkwrap, internalNames) {
	...
	for (const [lockPath, entry] of Object.entries(shrinkwrap.packages)) {
		...
		// Optional deps may be absent on this platform — only warn if they're present in
		// the lockfile but not in the shrinkwrap (which would indicate a bug in generation).
	}
...
function generateShrinkwrap() {
	...
	const rootDeps = Object.keys(codingAgentPackage.dependencies ?? {}).map((name) => ({ name, from: "", optional: false }));
	const rootOptDeps = Object.keys(codingAgentPackage.optionalDependencies ?? {}).map((name) => ({ name, from: "", optional: true }));
	const queue = [...rootDeps, ...rootOptDeps];
	...
	while (queue.length > 0) {
		...
		addExternalPackage(lockPackages, shrinkwrapPackages, addedPaths, queue, item.name, item.from, item.optional ?? false);
	}
