...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@entelligentsia/pi-ai`, `@entelligentsia/pi-agent-core`, `@entelligentsia/pi-coding-agent`, `@entelligentsia/pi-tui`, `typebox`.
