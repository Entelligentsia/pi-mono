...
Stateful agent with tool execution and event streaming. Built on `@entelligentsia/pi-ai`.

## Installation

```bash
npm install @entelligentsia/pi-agent-core
```
...
```typescript
import { Agent, streamProxy } from "@entelligentsia/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@entelligentsia/pi-agent-core";
